...
public class StatisticMenu implements MenuInterface{
    private HashMap<Player,Inventory>map = new HashMap<>();
    private Main plugin;
    public StatisticMenu(Main plugin){this.plugin=plugin;}
    @Override
    public void buildInventory(Player p) {
        Inventory inv = plugin.inventories.statisticInventory(p);
        map.put(p,inv);
        p.openInventory(inv);
    }
